...
namespace ycsbc {
    ...
    void my_rdma_db::Init() {
        ...
        pool_.conn.connect("192.168.200.51",8765);
        return ;
    }

    int my_rdma_db::Read(const string &table, const string &key, const vector<string> *fields, vector<KVPair> &result) {
        string key_index(table + key);
        result.clear();
        auto addr = static_cast<KVPair_for_RDMA_mempool*>(addrHashtable_->Get(key_index.c_str()));
        result.push_back(std::make_pair(addr->key(), addr->value()));
        printf("Read key: %s, value: %s\n", key_index.c_str(), addr->value());
        return DB::kOK;
    }

    int my_rdma_db::Scan(const string &table, const string &key, int len, const vector<string> *fields, vector<vector<KVPair>> &result) {
        string key_index(table + key);
        vector<AddrHashtable::KVPair> key_pairs = addrHashtable_->Entries(key_index.c_str(), len);
        result.clear();
        for (auto &key_pair : key_pairs) {
            auto addr = static_cast<KVPair_for_RDMA_mempool*>(key_pair.second);
    ...
    int my_rdma_db::Update(const string &table, const string &key, vector<KVPair> &values) {
        string key_index(table + key);
        auto addr = static_cast<KVPair_for_RDMA_mempool*>(addrHashtable_->Get(key_index.c_str()));
    ...
    int my_rdma_db::Insert(const string &table, const string &key, vector<KVPair> &values) {
        string key_index(table + key);
    ...
    int my_rdma_db::Delete(const string &table, const string &key) {
        string key_index(table + key);
        auto addr = static_cast<KVPair_for_RDMA_mempool*>(addrHashtable_->Get(key_index.c_str()));
